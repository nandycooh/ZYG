...
u16 Message_count_CRC_2(u8 CRC_Lenth,u8 CRC_Data[])
{
	...
	{
		Message_crc= Message_count_CRC_2(CRC_Lenth-1,CRC_Data)^CRC_Data[CRC_Lenth-1];
		for(j=0;j<8;j++)
		{
			DATA_MRight_bit = (uint8)(Message_crc & 0x0001);
			Message_crc = Message_crc >> 1;
			if(DATA_MRight_bit==1)
				Message_crc = Message_crc ^ 0xa001;
		}
	}
	return Message_crc;
}
