...
/************************************************************  
FileName: drv_SI4438.c   
Author: hf             
Date: 2021.03.23
Description: SI4438初始化及底层驱动        
Version: V 1.0  
Other:
***********************************************************/ 
...
/************************************************************  	
函数名称：	SI446x_Read_Response
函数功能：	SI446x读CTS和命令应答
入口参数：	pRead:返回数据首地址
            Length:长度
返回参数：	无
说明：	    无
...
/************************************************************  	
函数名称：	SI446x_Get_Part_Informatoin
函数功能：	SI446x读设备基本信息
入口参数：	pRead:返回数据首地址
返回参数：	无
说明：	    无
...
/************************************************************  	
函数名称：	SI446x_Get_Fun_Informatoin
函数功能：	SI446x读设备功能版本信息
入口参数：	pRead:返回数据首地址
返回参数：	无
说明：	    无
***********************************************************/
...
/************************************************************  	
函数名称：	SI446x_Get_Property
函数功能：	SI446x读取属性值
入口参数：	Group_Num	:	属性组(参考SI446X_PROPERTY)
            Num_Props	:	读取的属性个数
            pRead			:	返回数据首地址
返回参数：	无
说明：	    无
...
/************************************************************  	
函数名称：	SI446x_Get_Property_1
函数功能：	SI446x读取属性组1属性
入口参数：	Group_Num:开始的属性号(参考SI446X_PROPERTY)
返回参数：	无
说明：	    无
***********************************************************/
...
/************************************************************  	
函数名称：	SI446x_Config_Gpio
函数功能：	发送GPIO引脚配置命令并读取
入口参数：	GPIO0       配置GPIO0
            GPIO1       配置GPIO1
            GPIO2       配置GPIO2
            GPIO3       配置GPIO3.
...
/************************************************************  	
函数名称：	SI446x_Write_TxFifo
函数功能：	SI446x写TX FIFO
入口参数：	pWriteData：写数据首地址
            Length：数据长度
返回参数：	无
说明：	    无
***********************************************************/ 
...
/************************************************************  	
函数名称：	SI446x_Reset_RxFifo
函数功能：	SI446x 复位RX FIFO
入口参数：	void
返回参数：	无
说明：	    无
***********************************************************/ 
...
/************************************************************  	
函数名称：	SI446x_Reset_TxFifo
函数功能：	SI446x 复位TX FIFO
入口参数：	void
返回参数：	无
说明：	    无
***********************************************************/ 
...
/************************************************************  	
函数名称：	SI446x_Read_Packet
函数功能：	SI446x读RX FIFO数据
入口参数：	pRxData：数据首地址
返回参数：	数据个数
说明：	    无
***********************************************************/ 
...
/************************************************************  	
函数名称：	SI446x_Get_Packet_Information
函数功能：	SI446x读取当前数据包信息
入口参数：	pReadData：数据存放地址
            FieldNumMask：掩码域
            Length：长度
            DiffLen：不同长度
返回参数：	无
...
/************************************************************  	
函数名称：	SI446x_Get_Fifo_Information
函数功能：	SI446x读取FIFO状态
入口参数：	pReadData：数据存放地址
返回参数：	无
说明：	    无
...
/************************************************************  	
函数名称：	SI446x_Change_Status
函数功能：	SI446x更改状态命令
入口参数：	NextStatus：下一个状态
返回参数：	无
说明：	    无
***********************************************************/ 
...
uint8_t SI446x_Get_Device_Status( void )
{
   ...
   SI446x_Read_Response( l_Cmd, 3 );//l_Cmd[0]:响应有效字节流-l_Cmd[1]:设备状态-l_Cmd[2]:当前频道
   
   return l_Cmd[ 1 ] & 0x0F;
}

/************************************************************  	
函数名称：	SI446x_Set_Power
函数功能：	SI446x功率设置
入口参数：	PowerLevel：数据存放地址
返回参数：	设备当前状态
说明：	    无
***********************************************************/
...
/************************************************************  	
函数名称：	SI446x_Set_Data_Rate
函数功能：	SI446x的TX数据速率设置
入口参数：	DataRate：数据速率
返回参数：	设备当前状态
说明：	    无
***********************************************************/ 
...
void SI44xx_Power_Init(void)
{
	...
	GPIO_ResetBits(GPIOA, GPIO_Pin_3);//4438电源通
}

/************************************************************  	
函数名称：	SI446x_Gpio_Init
函数功能：	SI446x引脚初始化
入口参数：	void
返回参数：	无
说明：	    无
...
////待测试
//void Si4438_Channel_Set(u8 channel)
//{
//	Si4438_Driver_Port.SI4438_Start_Rx(channel, 0, 0,0,0,0);
//}

void SI4438Init( void )
{
	drv_spi_init( );//SPI初始化
...
void SI446x_Init( void )
{
	...
	SI4438Init();
//	SPI_ControlStruct.SI4438_Spi_Init();										//SPI初始化
//	SI4438_ControlStruct.SI4438_Gpio_Init();							//SI4438引脚初始化
//	SI4438_ControlStruct.SI4438_Reset();									//SI4438复位
//	//reset 后需要Power up设备 晶振30MHz
//	SI4438_ControlStruct.SI4438_Power_Up(RADIO_CONFIGURATION_DATA_RADIO_XO_FREQ);
//	SI4438_ControlStruct.SI4438_Config_Init();						//SI4438模块初始化
////	Si4438_Api_Port.SI4438_Set_DataRate(5000000);		//设置TX通信速率500kbps TESTOK 210218
//	SI4438_ControlStruct.SI4438_Set_Power(0x7f);					//功率设置
//	SI4438_ControlStruct.SI4438_Status_Set(RXTUNE_STATE);//切换到RX状态
//	while( RXTUNE_STATE != SI4438_ControlStruct.SI4438_Get_Device_Status());
//	SI4438_ControlStruct.SI4438_Start_Rx(SI4438_CHANNEL_SET, 0, PACKET_LENGTH,0,0,3);
//	
}

SI4438_ControlDef SI4438_ControlStruct={
