...
void VoidGear_Algorithm_Walk(short* gyro, short* accel)
{
	...
	if(avg_gyrof_start_last<1.0f&&avg_gyrof_start<1.0f)  //220228
	{
		count_j++;
		if(count_j>5000)
		{
	...
	if(count_j>=50&&avg_gyrof_start<1.0f) //220228
	{
		flag_Walk = 0;
	}
	if(avg_gyrof_start>=1.0) //220228
	{
		flag_Walk = 1;
//		printf("----------------------------------------------------\r\n");
	}
...
//	printf("accelf:%6.3f\t %6.3f\t %6.3f\r\n", accelf[0], accelf[1], accelf[2]);
	...
	avg_Speed = 0.8f*avg_Speed_gyrof + 0.2f*avg_Speed_accel ; //220228
