...
//static void Power_Switch(Work_Status status)
//{
//	if(status == ON){
//		GetHalAdapter_OptMode()->HAL_Restart();//软重启
//	}
//	else{
//		GetHalAdapter_OptMode()->HAL_RestoreDefaultSettings();
//		GetHalAdapter_OptMode()->HAL_EnterLowPowerStop();//停机模式，程序将暂停于此，唤醒时从此处接着运行
//	}
//}
...
void Walk_Task(void){
	...
	if(pF_Instance->MPU->isGenerateNewData()) 									//从寄存器获取温度
	{
		...
		{	
			pW_Instance->Algorithm_Walk(gyro,accel);								//脚步算法
			/*自定义协议*/
			pP_Instance->FmtHead();
			pP_Instance->FmtU8(SI433_ID);														//设备ID
			pP_Instance->FmtHex(quat,4);														//四元数
			pP_Instance->FmtU8(pW_Instance->GetWalk());							//步行标志位
			pP_Instance->FmtU16((uint16_t)pW_Instance->GetSpeed());	//步行速度
			pP_Instance->FmtU8(Turn_Statue);																	//精度
...
//	uint8_t batval = 0;															//电量判断
	...
	if(flash_config.pair_device.device_num == LEFTWALKID ||
		flash_config.pair_device.device_num == RIGHTWALKID )
		SI433_ID=flash_config.pair_device.device_num;
	else
		SI433_ID=RIGHTWALKID;
...
//#ifdef	TEST_SLEEP_FLAG
//		else	if(p_Flag.is_WaitingStartUp == true)//定时器任务
//		{ 
//			/*关机*/
//			if(p_Flag.is_TurnOffFlag==true)
//				POWER_ON_OFF=0;
...
//				printf("%f\r\n",calculate_bat());
				dgbugcount=0;
				if(SI433_ID==0x55)
					printf("walk:%d-speed:%f-channel:%dL-%x\r\n",pW_Instance->GetWalk(),pW_Instance->GetSpeed(),Si4438ChannelSet,p_Flag.is_TurnOffFlag);
				else if(SI433_ID==0x56)
					printf("walk:%d-speed:%f-channel:%dR-%x\r\n",pW_Instance->GetWalk(),pW_Instance->GetSpeed(),Si4438ChannelSet,p_Flag.is_TurnOffFlag);
			}
		}
	}
}
