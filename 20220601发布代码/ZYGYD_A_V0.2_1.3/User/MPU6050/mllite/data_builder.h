...
#define INV_NEW_DATA 64   //设置一组新的数据，如果没用就清除
/* Set if raw data exists */
#define INV_RAW_DATA 128   //如果原始数据存在则设置
/* Set if the sensor is on */
#define INV_SENSOR_ON 256  //如果传感器开启则设置
...
struct inv_single_sensor_t {
    ...
    */
    /*定位描述符。当旋转矩阵可以被认为只有0,1,1的元素时，描述如何从安装坐标系到机体坐标系。
      2位被用来描述1或-1的列，而第三位被用来表示符号。
      第8位是+/-1在第三行。6-7是+/-1的列在第三行。
      第5位是+/-1在第二行。第3-4是+/-1的列在第二行。
      第一行是+/-1的符号。0-1在第一行是+/-1的列。*/
    int orientation;
    ...
    * skipped due to power savings turning off this sensor.
    设置连续的数据。如果由于省电而关闭了这个传感器，就不会设置样本。
    * INV_NEW_DATA set for a new set of data, cleared if not available.
    设置一个新的数据，如果没有可用就清除
